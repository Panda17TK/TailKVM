...
use windows_sys::Win32::{
    ...
    UI::{
        Input::{
            Ime::{ImmGetCompositionStringW, ImmGetContext, ImmReleaseContext, GCS_RESULTSTR},
...
fn create_capture_window() -> Result<HWND, String> {
    ...
    // A real (activatable) 1x1 popup parked far off-screen: invisible in
    // practice, but able to take focus and host the thread's default IME
    // context, unlike a message-only window.
    let hwnd = unsafe {
        CreateWindowExW(
